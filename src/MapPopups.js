...
import { IS_MOBILE, ENABLE_COMMENTS } from './config/constants.js';
...
class MapPopups {
  ...
  showPOIPopup(e, iconSrc, poiType) {
    ...
    const poiTypeMapFallback = {
      'poi-bikeshop': 'Veikals/serviss (bez nosaukuma)',
      'poi-rental': 'Velo stacija (bez nosaukuma)',
    };
    ...
    let html = `
            <div>
                ...
                <div class="relative inline-block mt-3 group">
                    ...
                    >
                        ${layer.shortName ?? layer.displayName ?? layer.name}
                    </div>
                    ${
                      layer.description
                        ? `
                ...
                <div class="md:text-2xl text-lg mt-2 md:mb-5 mb-3 tracking-tight">
                    ${
                      properties.name
                        ? properties.name
                        : '<span class="italic opacity-50">Iela bez nosaukuma</span>'
                    }
                </div>
